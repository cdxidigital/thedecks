...
export function DJWorkspace() {
  ...
  return <main className="app-shell">
    ...
    <section className="lower-grid">
      ...
      <section className="library-panel"><header className="library-header"><div><p className="eyebrow"><Library /> UNIFIED LIBRARY</p><h2>All Tracks</h2></div><label className="search"><Search /><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search tracks, artists, keys…" /></label><button className="secondary-action" onClick={()=>setRemixOpen(true)}><WandSparkles /> Remix</button></header><div className="track-table" role="table"><div className="track-row head" role="row"><span>#</span><span>TITLE</span><span>BPM</span><span>KEY</span><span>ENERGY</span><span>SOURCE</span><span>LOAD</span></div>{filtered.map((t,i)=><button role="row" className={`track-row ${selected.title===t.title?'selected':''}`} key={t.title} onClick={()=>setSelected(t)}><span>{String(i+1).padStart(2,'0')}</span><span><b>{t.title}</b><small>{t.artist} · {t.time}</small></span><span>{t.bpm}</span><span className="key-pill">{t.key}</span><span><i className="energy"><i style={{width:`${t.energy}%`}}/></i></span><span>{t.source}</span><span className="load-actions" onClick={e=>e.stopPropagation()}>{Array.from({length:deckCount}).map((_,d)=><button key={d} onClick={()=>loadTrack(d,t)}>{String.fromCharCode(65+d)}</button>)}</span></button>)}</div></section>
      <aside className="ai-panel"><header><span><Bot /></span><div><p>PERFORMANCE ASSISTANT</p><b>Pulse AI is listening</b></div><i /></header><div className="ai-energy"><div className="energy-ring"><b>86</b><small>ENERGY</small></div><div><b>Room is building</b><p>Maintain tension for 32 bars, then lift.</p></div></div><div className="recommendation"><span>NEXT TRACK · 94% FIT</span><h3>Low Orbit</h3><p>Vanta & Rue · 126 BPM · 9A</p><div className="fit-row"><i>KEY +1</i><i>ENERGY +9</i><i>PHRASE ✓</i></div><button onClick={()=>{loadTrack(1,tracks[3]);setNotice('AI recommendation loaded to Deck B')}}>Load to Deck B</button></div><div className="insight"><Sparkles /><p><b>{notice}</b><span>Try a 16-beat loop and cut lows at the next phrase.</span></p></div><button className="ask-ai"><Mic2 /> Ask Pulse AI <ChevronDown /></button></aside>
    </section>
    <footer><span>CPU <b>18%</b></span><span>AUDIO <b>3.2 ms</b></span><span>DROPOUTS <b>0</b></span><span className="ml-auto">PulseMix Engine <b>Online</b></span></footer>
