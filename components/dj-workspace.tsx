...
const demoTracks: Track[] = [
  { title: 'Neon Current', artist: 'Mira Sol', bpm: 124, key: '8A', energy: 82, time: '5:42', source: 'Demo', color: 'cyan' },
  { title: 'After Hours Signal', artist: 'North Phase', bpm: 126, key: '9A', energy: 88, time: '6:18', source: 'Demo', color: 'amber' },
  { title: 'Glass Cities', artist: 'Aiko Drift', bpm: 122, key: '8B', energy: 71, time: '4:57', source: 'Demo', color: 'cyan' },
  { title: 'Low Orbit', artist: 'Vanta & Rue', bpm: 126, key: '9A', energy: 91, time: '5:33', source: 'Demo', color: 'amber' },
]
...
export function DJWorkspace() {
  ...
  const [decks, setDecks] = useState<DeckState[]>(initialDecks)
  ...
  // useEffect runs only on the client — avoids SSR/hydration time mismatch
  useEffect(() => {
  ...
  return <main className="app-shell">
    ...
    <section className="lower-grid">
      <aside className="sources"><span className="section-label">SOURCES</span>{[[Library,'Tracks',library.length.toString()],[ListMusic,'Playlists','0'],[Cloud,'Cloud Drive',''],[Radio,'Streaming','0']].map(([Icon,label,count],i)=>{const I=Icon as typeof Library;return <button key={label as string} className={i===0?'active':''}><I /><span>{label as string}</span><small>{count as string}</small></button>})}<div className="source-divider"/><p>STATUS</p><button><i className="online" /><span>Suno Remix API</span></button></aside>
      <section className="library-panel"><header className="library-header"><div><p className="eyebrow"><Library /> TRACK LIBRARY</p><h2>{library.length === 0 ? 'No tracks imported' : `${library.length} track${library.length === 1 ? '' : 's'}`}</h2></div><label className="search"><Search /><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search tracks, artists, keys…" disabled={library.length === 0} /></label><button className="secondary-action" onClick={()=>{if(selected) setRemixOpen(true)}} disabled={!selected}><WandSparkles /> Remix</button></header>{library.length === 0 ? <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100%',color:'var(--muted-foreground)',textAlign:'center',flexDirection:'column',gap:'12px'}}><p style={{margin:0,fontSize:'12px'}}>Import or create tracks to begin</p><small style={{margin:0,color:'var(--muted-foreground)'}}>Add tracks to your library to load them onto decks and generate remixes with Suno AI.</small><button onClick={() => { setLibrary(demoTracks); setSelected(demoTracks[0]); setNotice('Demo tracks loaded. Ready to remix!'); }} style={{marginTop:'8px',padding:'6px 12px',fontSize:'11px',background:'var(--primary)',color:'var(--background)',border:'none',borderRadius:'4px',cursor:'pointer',fontWeight:'500'}}>Load demo tracks</button></div> : <div className="track-table" role="table"><div className="track-row head" role="row"><span>#</span><span>TITLE</span><span>BPM</span><span>KEY</span><span>ENERGY</span><span>SOURCE</span><span>LOAD</span></div>{filtered.map((t,i)=><div role="row" tabIndex={0} className={`track-row ${selected?.title===t.title?'selected':''}`} key={t.title} onClick={()=>setSelected(t)} onKeyDown={e=>{if((e.key==='Enter'||e.key===' ')&&!e.nativeEvent.isComposing&&e.keyCode!==229){e.preventDefault();setSelected(t)}}}><span>{String(i+1).padStart(2,'0')}</span><span><b>{t.title}</b><small>{t.artist} · {t.time}</small></span><span>{t.bpm}</span><span className="key-pill">{t.key}</span><span><i className="energy"><i style={{width:`${t.energy}%`}}/></i></span><span>{t.source}</span><span className="load-actions" onClick={e=>e.stopPropagation()}>{Array.from({length:deckCount}).map((_,d)=><button key={d} onClick={()=>loadTrack(d,t)} aria-label={`Load ${t.title} to Deck ${String.fromCharCode(65+d)}`}>{String.fromCharCode(65+d)}</button>)}</span></div>)}</div>}</section>
    ...
    <footer><span>VERSION <b>1.0</b></span><span>REMIX <b>ENABLED</b></span><span>TRACKS <b>{library.length}</b></span><span className="ml-auto">{'READY FOR DEPLOYMENT'} <b>LIVE</b></span></footer>
